...
describe('Pet sex', () => {
  it('Should add animal sex', async () => {
  ...
  it('Should show all sex types', async () => {
    const response = await request(app).get('/sexs')

    expect(response.body).toEqual([{ id: 1, name: 'MASCULINO' }])
  })

  it('Should show one specific pet sex', async () => {
    const response = await request(app).get('/sex/1')

    expect(response.body).toEqual({ id: 1, name: 'MASCULINO' })
  })

  it('Should uptdate pet sex', async () => {
  ...
  it('Should delete a animal sex', async () => {
